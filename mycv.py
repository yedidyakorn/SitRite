...
def checkImg(frameinput):
    ...
    frameWidth = frameinput.shape[1]
    frameHeight = frameinput.shape[0]
    threshold = 0.1
    ...
    inWidth = 368
    inHeight = 368
    inpBlob = cv2.dnn.blobFromImage(frameinput, 1.0 / 255, (inWidth, inHeight),
    ...
    for pair in POSE_PAIRS:
        ...
        if points[partA] and points[partB]:
            cv2.line(frameinput, points[partA], points[partB], (0, 255, 255), 2)
            cv2.circle(frameinput, points[partA], 8, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)

    """
    cv2.imshow('Output-Keypoints', frameCopy)
    cv2.imshow('Output-Skeleton', frameinput)

    cv2.imwrite('Output-Keypoints.jpg', frameCopy)
    cv2.imwrite('Output-Skeleton.jpg', frameinput)
    """
